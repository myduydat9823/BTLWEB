...
public class EntryService : IEntryService
{
  ...
  public async Task<OperationResult> SubmitAsync(SubmitEntryViewModel model, int userId)
  {
    try
    {
      ...
      var alreadySubmitted = await _entryRepository.UserHasEntryInCompetitionAsync(userId, model.CompetitionId);
      if (alreadySubmitted)
      {
      ...
      var photo = new Photo
      {
        ...
        UserId = userId,
        FileSize = model.PhotoFile.Length,
      ...
      _logger.LogInformation("User {UserId} submitted entry {EntryId} to competition {CompetitionId}.",
          userId, entry.Id, model.CompetitionId);

      return OperationResult.Success("Gửi bài dự thi thành công.");
    }
    catch (Exception ex)
    {
